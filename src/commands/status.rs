    ...
    loop {
        ...
        if event::poll(std::time::Duration::from_millis(1000)).unwrap_or(false) {
            if let Ok(event::Event::Key(key)) = event::read() {
                match key.code {
                    event::KeyCode::Char('q') | event::KeyCode::Char('b') | event::KeyCode::Esc => {
                        break
                    }
                    _ => {}
                }
...
fn build_status(sys: &System) -> String {
    ...
    let total_cpu: f32 =
        sys.cpus().iter().map(|c| c.cpu_usage()).sum::<f32>() / cpu_cores.max(1) as f32;
    ...
    let health_dot = if health >= 80 {
        "\x1b[32m\u{25cf}\x1b[0m"
    } else if health >= 50 {
        "\x1b[33m\u{25cf}\x1b[0m"
    } else {
        "\x1b[31m\u{25cf}\x1b[0m"
    };
    ...
    // System info line
    o.push_str(&format!(
        "  {} Health \x1b[1m{}\x1b[0m  \x1b[90m{} \u{b7} macOS {} \u{b7} up {}d {}h\x1b[0m\r\n",
    ...
    o.push_str(&format!(
        "  Free {} {:>3}%  \x1b[32m{}\x1b[0m\r\n",
        bar_green(100 - mem_pct, 100),
        100 - mem_pct,
        ByteSize::b(free_mem)
    ));
    ...
    if swap_total > 0 {
        ...
        o.push_str(&format!(
            ...
            bar(sp, 100),
            sp,
            ByteSize::b(swap_used),
            ByteSize::b(swap_total)
        ));
    }
    o.push_str("\r\n");
    ...
    if let Some(batt) = get_battery_info() {
        ...
        o.push_str(&format!(
            "  {} {} {:>3}%  \x1b[90m{} cycles\x1b[0m\r\n",
            batt.status,
            bar_green(batt.level, 100),
            batt.level,
            batt.cycles
        ));
        o.push_str("\r\n");
...
fn get_battery_info() -> Option<BatteryInfo> {
    ...
    let level = stdout
        ...
        .unwrap_or(0);
    let status = if stdout.contains("charging") {
        "\u{26a1} Charging"
