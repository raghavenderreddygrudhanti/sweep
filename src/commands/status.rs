...
fn build_status(sys: &System) -> String {
    ...
    // Header
    o.push_str(&format!("\r\n  \x1b[1mStatus\x1b[0m  \x1b[32m●\x1b[0m \x1b[1m{}\x1b[0m  {} · {}cores · {} · up {}d{}h\r\n",
        health, host, cpu_cores, ByteSize::b(total_mem), days, hours));
    o.push_str("\r\n");

    // CPU
    o.push_str(&format!("  \x1b[33mCPU\x1b[0m   {} {:>4.0}%\r\n", bar(total_cpu as u64, 100), total_cpu));
    for (i, cpu) in sys.cpus().iter().take(4).enumerate() {
        let u = cpu.cpu_usage();
        o.push_str(&format!("    C{} {} {:>4.0}%\r\n", i+1, bar(u as u64, 100), u));
    }
    if cpu_cores > 4 { o.push_str(&format!("    \x1b[90m(+{} more)\x1b[0m\r\n", cpu_cores - 4)); }
    o.push_str("\r\n");

    // Memory
    o.push_str(&format!("  \x1b[33mMEM\x1b[0m   {} {:>3}%  {}/{}\r\n",
    ...
    if swap_total > 0 {
        ...
        o.push_str(&format!("  \x1b[33mSWP\x1b[0m   {} {:>3}%  {}/{}\r\n",
    ...
    for disk in disks.list() {
        if disk.mount_point().to_string_lossy() == "/" {
            ...
            o.push_str(&format!("  \x1b[33mDISK\x1b[0m  {} {:>3}%  \x1b[32m{} free\x1b[0m\r\n",
    ...
    if let Some(batt) = get_battery_info() {
        o.push_str(&format!("  \x1b[33mBAT\x1b[0m   {} {:>3}%  \x1b[32m{}\x1b[0m · {} cycles\r\n",
            bar(batt.level, 100), batt.level, batt.status, batt.cycles));
        o.push_str("\r\n");
    }

    // Processes
    o.push_str("  \x1b[33mPROCS\x1b[0m\r\n");
    ...
    for (name, cpu, mem) in procs.iter().take(5) {
        ...
        o.push_str(&format!("    {:<14} {:>5.1}% {:>8}{}\r\n", n, cpu, ByteSize::b(*mem), hot));
    }
    o.push_str("\r\n");

    // Footer
    o.push_str("  \x1b[90m─────────────────────────────────\x1b[0m\r\n");
    o.push_str("  \x1b[90mPress q to quit · refreshes every 1s\x1b[0m\r\n");
