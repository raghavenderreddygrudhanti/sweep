...
pub fn run(dry_run: bool, mode: DeleteMode) {
    ...
    println!("  \x1b[90mFree space: {}\x1b[0m\n", ByteSize::b(free_before));
    ...
    for category in &categories {
        ...
        for item in &category.items {
            if !item.path.exists() { continue; }
            if !item.safe { continue; } // Skip items needing review in auto-clean
            ...
            if size > 1000 {
                ...
                let count = std::fs::read_dir(&item.path).map(|d| d.count()).unwrap_or(0);
                if count > 0 {
                    println!("    \x1b[32m\u{2713}\x1b[0m {} {} items, \x1b[32m{}\x1b[0m",
                        item.label, count, ByteSize::b(size));
                } else {
                    println!("    \x1b[32m\u{2713}\x1b[0m {}, \x1b[32m{}\x1b[0m",
                        item.label, ByteSize::b(size));
                }
        ...
        for item in &category.items {
            ...
            let size = scanner::scan_size_native(&item.path);
            if size > 50 * 1024 * 1024 { // Only show review items > 50MB
                if !category_found {
                    println!("  \x1b[1;32m\u{25b8} {}\x1b[0m", category.name);
                    category_found = true;
                }
                println!("    \x1b[90m\u{25cb} {} \u{2014} manual review ({})\x1b[0m",
                    item.label, ByteSize::b(size));
            }
        }
        ...
        if category_found {
            ...
            if category_size > 0 { total_categories += 1; }
            println!();
        }
    }
    ...
    if !orphans.is_empty() {
        ...
        for orphan in orphans.iter().take(10) {
            ...
            println!("    \x1b[90m\u{2022}\x1b[0m {} \x1b[90m({}, {} days old)\x1b[0m",
                display, ByteSize::b(orphan.size), orphan.age_days);
    ...
    if dry_run {
        println!("  \x1b[1;33mWould free: {}\x1b[0m ({} items across {} categories)",
            ByteSize::b(total_freed), total_items, total_categories);
    ...
    println!("  \x1b[1;32mWould free: {}\x1b[0m ({} items, {} categories)\n",
        ByteSize::b(total_freed), total_items, total_categories);
    ...
    for (_, size, label) in sorted.iter().take(5) {
        println!("    \x1b[90m\u{2022}\x1b[0m {:>9}  {}", ByteSize::b(*size), label);
    }
    println!();
    ...
    for (path, _size, label) in &targets {
        ...
        if let Ok(entries) = std::fs::read_dir(path) {
            for entry in entries.flatten() {
                ...
                if ok { item_freed += sz; } else { failed += 1; }
            }
        }

        print!("\r\x1b[K");
        if item_freed > 0 && failed == 0 {
            println!("  \x1b[32m\u{2713}\x1b[0m {} \u{2014} freed {}", label, ByteSize::b(item_freed));
        } else if item_freed > 0 {
            println!("  \x1b[32m\u{2713}\x1b[0m {} \u{2014} freed {} \x1b[90m({} skipped)\x1b[0m",
                label, ByteSize::b(item_freed), failed);
        } else if failed > 0 {
            println!("  \x1b[90m\u{2013}\x1b[0m {} \x1b[90m\u{2014} skipped (protected)\x1b[0m", label);
        }
    ...
    let actual_change = if free_after > free_before { free_after - free_before } else { actually_freed };
    ...
    println!("  Tracked cleanup: \x1b[1m{}\x1b[0m | Items: {} | Categories: {}",
        ByteSize::b(actually_freed), total_items, total_categories);
    println!("  Free space change: \x1b[32m+{}\x1b[0m", ByteSize::b(actual_change));
    println!("  Free space now: \x1b[1m{}\x1b[0m\n", ByteSize::b(free_after));
...
fn check_running_browsers() {
    ...
    for (proc_name, display_name) in &browsers {
        let running = sys.processes().values().any(|p| {
            p.name().to_lowercase().contains(proc_name)
        });
        if running {
            if !any_running {
            ...
            println!("    \x1b[90m\u{25cb} {} is running \u{2014} profile cache cleanup skipped\x1b[0m", display_name);
        }
    }
    if any_running { println!(); }
}
