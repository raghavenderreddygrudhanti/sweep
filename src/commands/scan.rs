...
fn get_categories() -> Vec<Category> {
    ...
    vec![
        ...
        Category {
            name: "Docker",
            path: home.join("Library/Containers/com.docker.docker"),
            size: -1,
            cleanable: true,
            color: "34",
        },
...
pub fn run(_path: &str) {
    ...
    let pending_paths: Vec<(usize, PathBuf)> = categories
        .iter()
        .enumerate()
        .filter(|(_, c)| c.size < 0)
    ...
    let _handles: Vec<_> = pending_paths
        .iter()
        .map(|(idx, path)| {
            let path = path.clone();
            let idx = *idx;
    ...
    loop {
        // Check for completed background scans
        {
            ...
            for (&idx, &size) in res.iter() {
                if idx < categories.len() {
                    ...
                    // Save to cache
                    cache::save_size(categories[idx].path.to_str().unwrap_or(""), size);
                }
            }
            res.clear();
        }
        ...
        out.push_str(&format!(
            "  \x1b[90m{} free\x1b[0m{}\r\n\r\n",
            ByteSize::b(free_space),
            scan_indicator
        ));

        if mode == "overview" {
            // Separate ready vs pending entries
            let mut ready: Vec<(usize, &Category)> = categories
                .iter()
                .enumerate()
                .filter(|(_, c)| c.size > 0) // Only show items with actual size > 0
                .collect();
            let pending: Vec<(usize, &Category)> = categories
                .iter()
                .enumerate()
            ...
            for (display_idx, (_orig_idx, cat)) in ready.iter().enumerate() {
                ...
                let ptr = if display_idx == selected {
                    " \x1b[32m▶\x1b[0m"
                } else {
                    "  "
                };
                let cleanable = if cat.cleanable {
                    "\x1b[33m··\x1b[0m"
                } else {
                    "  "
                };
            ...
            for (i, result) in folder_results.iter().take(15).enumerate() {
                ...
                let bar = format!(
                    "█{}░{}",
                    "█".repeat(bar_len.saturating_sub(1)),
                    "░".repeat(12usize.saturating_sub(bar_len))
                );
                ...
                out.push_str(&format!(
                    ...
                    sel_icon,
                    bar,
                    ByteSize::b(result.size).to_string(),
                    icon,
                    name_fmt
                ));
            }
            ...
            if let Ok(Event::Key(key)) = event::read() {
                        ...
                        super::ui::NavAction::Char('y') | super::ui::NavAction::Char('Y') => {
                            if mode == "folder" && selected < folder_results.len() {
                                ...
                                if deleted {
                                    ...
                                    if selected >= folder_results.len() && selected > 0 {
                                        selected -= 1;
                                    }
                                } else {
                ...
                match action {
                    ...
                    super::ui::NavAction::Select => {
                        if mode == "overview" {
                            let ready: Vec<(usize, &Category)> = categories
                                .iter()
                                .enumerate()
                                .filter(|(_, c)| c.size > 0)
                                .collect();
                            ...
                            if folder_results[selected].is_dir {
                                ...
                                let name = PathBuf::from(&folder_results[selected].path)
                                    ...
                                    .to_string();
                                status_msg = format!(
                                    "\x1b[90m\u{2139} {} is a file (use d to delete)\x1b[0m",
                                    name
                                );
                            }
                        }
                    ...
                    super::ui::NavAction::Back => {
                        if mode == "folder" {
                            let is_root = categories
                                .iter()
                                .filter(|c| c.size > 0)
                                .any(|c| c.path == current_path)
                            ...
                            if is_root {
                                ...
                                let parent_is_root = categories
                                    .iter()
                                    .filter(|c| c.size > 0)
                                    .any(|c| c.path == parent_buf)
