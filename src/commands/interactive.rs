...
pub fn run() {
    ...
    loop {
        ...
        for (i, (label, desc, color)) in MENU.iter().enumerate() {
            if i == selected {
                out.push_str(&format!(
                    "  \x1b[1;32m\u{25b6}\x1b[0m {}\x1b[1m{:<12}\x1b[0m \x1b[37m{}\x1b[0m\r\n",
                    color, label, desc
                ));
            } else {
                out.push_str(&format!(
                    "    {}{:<12}\x1b[0m \x1b[90m{}\x1b[0m\r\n",
                    color, label, desc
                ));
            }
        }
        ...
        if event::poll(std::time::Duration::from_millis(600)).unwrap_or(false) {
            if let Ok(Event::Key(key)) = event::read() {
                ...
                match action {
                    ...
                    super::ui::NavAction::Char('1') => {
                        selected = 0;
                    }
                    super::ui::NavAction::Char('2') => {
                        selected = 1;
                    }
                    super::ui::NavAction::Char('3') => {
                        selected = 2;
                    }
                    super::ui::NavAction::Char('4') => {
                        selected = 3;
                    }
                    super::ui::NavAction::Char('5') => {
                        selected = 4;
                    }
                    super::ui::NavAction::Char('6') => {
                        selected = 5;
                    }
                    super::ui::NavAction::Char('7') => {
                        selected = 6;
                    }
                    super::ui::NavAction::Char('8') => {
                        selected = 7;
                    }
                    super::ui::NavAction::Char('9') => {
                        selected = 8;
                    }
...
fn show_more_options() {
    ...
    println!(
        "  \x1b[32m{:<28}\x1b[0m {}",
        "sweep clean", "Free up disk space (all caches)"
    );
    println!(
        "  \x1b[32m{:<28}\x1b[0m {}",
        "sweep ai", "Clean AI/ML model caches"
    );
    println!(
        "  \x1b[32m{:<28}\x1b[0m {}",
        "sweep dev", "Remove old build artifacts"
    );
    println!(
        "  \x1b[32m{:<28}\x1b[0m {}",
        "sweep docker", "Clean Docker images and volumes"
    );
    println!(
        "  \x1b[32m{:<28}\x1b[0m {}",
    ...
    println!(
        "  \x1b[32m{:<28}\x1b[0m {}",
        "sweep uninstall", "Remove apps completely"
    );
    println!();
    println!("  \x1b[1mANALYSIS\x1b[0m");
    println!(
        "  \x1b[34m{:<28}\x1b[0m {}",
        "sweep scan [path]", "Interactive disk explorer"
    );
    println!(
        "  \x1b[34m{:<28}\x1b[0m {}",
        "sweep recommend", "Smart cleanup suggestions"
    );
    println!(
        "  \x1b[34m{:<28}\x1b[0m {}",
        "sweep timeline", "What grew/shrank since last scan"
    );
    println!(
        "  \x1b[34m{:<28}\x1b[0m {}",
    ...
    println!(
        "  \x1b[34m{:<28}\x1b[0m {}",
        "sweep history", "Past cleanup operations"
    );
    println!();
    ...
    println!(
        "  \x1b[33m{:<28}\x1b[0m {}",
        "sweep completion <shell>", "Shell tab completion setup"
    );
    println!();
    ...
    println!(
        "  \x1b[90m{:<28}\x1b[0m {}",
        "--older-than <days>", "Min age for dev artifacts (default: 7)"
    );
    println!();
